...
<script setup>
    ...
    async function init() {
        ...
        const links = [], initVals = [];
        sim.data = []

        props.data.forEach(d => {
            initVals.push(props.xScale(access.x(d)) - radius.value + 10);
            initVals.push(initVals[initVals.length-1]);
            sim.data.push(Object.assign({
                x: initVals[initVals.length-1],
                y: radius.value,
                fy: radius.value,
                r: radius.value
            }, d))
